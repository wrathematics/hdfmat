...
extern "C" SEXP R_hdfmat_read(SEXP row_start_, SEXP row_stop_, SEXP m_, SEXP n_, SEXP ds, SEXP type)
{
  SEXP ret;
  ...
  const hsize_t len = row_stop - row_start + 1;
  
  if (INT(type) == TYPE_DOUBLE)
  {
    PROTECT(ret = allocMatrix(REALSXP, n, len));
    TRY_CATCH( read(row_start, row_stop, m, n, REAL(ret), dataset, H5::PredType::IEEE_F64LE) );
  }
  else // if (INT(type) == TYPE_FLOAT)
  {
    PROTECT(ret = allocMatrix(INTSXP, n, len));
    TRY_CATCH( read(row_start, row_stop, m, n, FLOAT(ret), dataset, H5::PredType::IEEE_F32LE) );
  }
